...
case "$ROOT_DIR" in
  /mnt/c/*|/mnt/d/*|/mnt/e/*)
    printf '[WARN] Repository berada di /mnt. Pindahkan ke ~/src/mcsos.\n'
    ;;
  *)
    printf '[OK] Lokasi repository benar.\n'
    ;;
esac
...
if [ "$fail" -ne 0 ]; then
  say "Environment check failed"
  exit 1
fi

say "Environment check completed"
