...
rm -f "$LOG"
mkdir -p build
QEMU_ARGS=(-machine q35-cpu qemu64-m 512M-serial "file:$LOG"-display none-monitor none-no-reboot-no-shutdown-drive "if=pflash,format=raw,readonly=on,file=$OVMF_CODE"-cdrom "$ISO"
)
